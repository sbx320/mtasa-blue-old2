...
{
    CEntitySA* pCreatorSA = dynamic_cast <CEntitySA*> ( pCreator );
    if ( pCreatorSA )
        internalInterface->pEntProjectileOwner = pCreatorSA->GetInterface ( );
}

void CProjectileInfoSA::SetWeaponType ( eWeaponType weaponType )
{
    internalInterface->dwProjectileType = weaponType;
}